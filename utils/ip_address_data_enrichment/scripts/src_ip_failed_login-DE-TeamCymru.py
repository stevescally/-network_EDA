...
def cymru_dns_q(_ip_addrs):
    ...
    1.  Loop through each IP address in the source IP address list and build
        an entry of reversed octect v4/v6 query to the respective IPv4 and
        IPv6 endpoints.
    ...
    2.  Check the return code of the query as all IP addresses may not have an
        entry or a lookup can timeout. We need to create a row entry regardless
        so that the index mapping of the dataframes is consistant. This allows
        us to merge the dataframes later into one.

    3a. With a successful response code we can parse the returned answer
        into the information we want.

        We extract the ASN associated with the returned IP adddress query
        and perform a lookup to the ASN domain endpoint (.asn.cymru.com)

        We extract the list of AS peers as seen through BGP from the
        respective IP address.

    3b. If the response code is unsucessful we fill the values with the
        numpy not-a-number(NaN) value. This maintains the n-dementional
        array size between the original source and returned query data.

    4.  With the inital IP address query data, AS Description and list
        of peer AS's a dictionary entry is created for each IP address
        queried. The list is returned and stored as a pandas dataframe.
    '''
    ...
    for ip in track(_ip_addrs, "Querying IP Data"):
        ...
        if query_ip_response.rcode() == 0:
            ...
            # 0 - ASN, 1 - BGP Prefix, 2 - Country Code, 3 - RIR,
            # 4 - Allocation Date, 5 - AS Description, 6 - AS Peers
            lookup_list.append({'ASN': query_ip_answer[0].strip(),
                                'BGP Prefix': query_ip_answer[1].strip(),
                                'Country Code': query_ip_answer[2].strip(),
                                'RIR': query_ip_answer[3].strip(),
                                'Allocation Date': query_ip_answer[4].strip(),
                                'AS Description': query_asn_answer[4].strip(),
            ...
            lookup_list.append({'ASN': np.NaN,
                                'BGP Prefix': np.NaN,
                                'Country Code': 'None',
                                'RIR': 'None',
                                'Allocation Date': np.NaN,
                                'AS Description': 'None',
                                'AS Peers': np.NaN})
...
def main():
    ...
    df_ip_address = pd.read_csv(os.path.abspath(args.input_file),
                                delim_whitespace=1,
    ...
    all_data.to_csv(os.path.abspath(args.output_file),
                    index=False)
